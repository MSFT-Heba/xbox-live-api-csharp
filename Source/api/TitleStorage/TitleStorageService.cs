...
namespace Microsoft.Xbox.Services.TitleStorage
{
    ...
    public class TitleStorageService: ITitleStorageService
    {
        ...
        internal static readonly Uri TitleStorageBaseUri = new Uri("https://titlestorage.xboxlive.com");
        ...
        public IHttpRequestHandler HttpRequestHandler;
        ...
        public Task<TitleStorageQuota> GetQuotaAsync(XboxLiveUser user, TitleStorageType storageType)
        {
            ...
            var httpRequest = this.HttpRequestHandler.Create(HttpMethod.Get, TitleStorageBaseUri.ToString(), subQuery);
            httpRequest.ContractVersion = TitleStorageApiContract;

            return this.HttpRequestHandler.GetResponseWithAuth(httpRequest, user).ContinueWith(
        ...
        public Task DeleteBlobAsync(XboxLiveUser user, TitleStorageBlobMetadata blobMetadata, BlobQueryProperties blobQueryProperties)
        {
            ...
            var httpRequest = this.HttpRequestHandler.Create(HttpMethod.Delete, TitleStorageBaseUri.ToString(), subPathAndQueryResult);
            httpRequest.ContractVersion = TitleStorageApiContract;
            httpRequest.ContentType = ContentTypeHeaderValue;
            
            this.SetEtagHeader(httpRequest, blobMetadata.ETag, eTagMatchCondition);
            return this.HttpRequestHandler.GetResponseWithAuth(httpRequest, user).ContinueWith(responseTask =>
            {
            ...
            BlobQueryProperties blobQueryProperties)
        {
            ...
            while (isDownloading)
            {
                var httpRequest = this.HttpRequestHandler.Create(HttpMethod.Get, TitleStorageBaseUri.ToString(), subPathAndQueryResult);
                httpRequest.ContractVersion = TitleStorageApiContract;
                httpRequest.ContentType = ContentTypeHeaderValue;
                httpRequest.longHttpCall = true;

                this.SetEtagHeader(httpRequest, blobMetadata.ETag, blobQueryProperties.ETagMatchCondition);

                if (isBinaryData)
                {
                    this.HttpRequestHandler.SetRangeHeader(httpRequest, startByte, startByte + preferredDownloadBlockSize);
                }

                this.HttpRequestHandler.GetResponseWithAuth(httpRequest, user).ContinueWith(responseTask =>
                {
        ...
        public Task<TitleStorageBlobMetadata> UploadBlobAsync(XboxLiveUser user, TitleStorageBlobMetadata blobMetadata, List<byte> blobBuffer, BlobQueryProperties blobQueryProperties)
        {
            ...
            while (start < blobBuffer.Count)
            {
                ...
                var httpRequest = this.HttpRequestHandler.Create(HttpMethod.Put, TitleStorageBaseUri.ToString(), subpathAndQueryResult);
                httpRequest.ContractVersion = TitleStorageApiContract;
                httpRequest.ContentType = ContentTypeHeaderValue;
                httpRequest.longHttpCall = true;
            ...
            string continuationToken = "")
        {
            ...
            var httpRequest = this.HttpRequestHandler.Create(HttpMethod.Get, TitleStorageBaseUri.ToString(), subPathAndQueryResult);
            httpRequest.ContractVersion = TitleStorageApiContract;
            return this.HttpRequestHandler.GetResponseWithAuth(httpRequest, user)
                .ContinueWith(
            ...
            XboxLiveHttpRequest httpRequest, string eTag, TitleStorageETagMatchCondition eTagMatchCondition)
        {
            if (eTagMatchCondition != TitleStorageETagMatchCondition.NotUsed)
            {
                ...
                {
                    this.HttpRequestHandler.SetCustomHeader(httpRequest, ETagHeaderName, eTag);
                    var headerToUse = eTagMatchCondition == TitleStorageETagMatchCondition.IfMatch ? IfMatchHeaderName : IfNoneHeaderName;
                    this.HttpRequestHandler.SetCustomHeader(httpRequest, headerToUse, eTag);
                }
            }
        }
    }
